...
module.exports = (function() {
	...
	let ws = {
		...
		http : {
			...
			resCookieForUser : (res, rs, newToken) => {
				ws.http.resCookie(res, "userid", rs.USER_ID) //session or persist는 클라이언트에서 다시 한번 판단하기로 하고 여기는 모두 세션 쿠키로 내림
                ws.http.resCookie(res, "token", newToken) //jwt
