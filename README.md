...
# 주요 특징 (메시징 관점에서)
   ...
   사용하는 경우 Noti가 둘 다 동시에 오면 사용자가 불편하게 됩니다. 예를 들면, 둘 다 Noti를 표시했다가<br/>
   한쪽에서 읽으면 다른 한쪽의 Noti는 제거하면 만일 웹을 위주로 사용하고 있는 상태라면 계속해서 앱으로<br/>
   Noti 소리나 진동이 울리는 상황이 됩니다. 그래서, 아래와 같은 방식으로 Noti를 처리하고자 했습니다.<br/>
   ...
   - 그런데, 앱에서 챗방이 열려 있고 포커싱이 있는 상태라면 웹으로 톡 올 때마다 노티가 계속 표시되었다가<br/>
     제거되는 것이 반복되는데 이것을 해결하기 위해 웹에서 focusedRoomid라는 변수로 앱으로 현재 스택<br/>
     맨 위에 열려 있고 스크린도 On 상태라고 인지되는 roomid를 받아 해당 roomid가 맞으면 웹에서는<br/>
