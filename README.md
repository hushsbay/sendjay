...
# 주요 특징 (메시징 관점에서)
   ...
   웹과 앱 동시에 사용하는 경우 Noti가 둘 다 동시에 오면 사용자가 불편하게 됩니다.<br/>
   이 경우, 둘 다 Noti를 표시하고 한쪽에서 읽으면 다른 한쪽의 Noti는 제거하면<br/>
   예를 들어 웹을 위주로 사용하고 있는 상태라면 계속해서 앱으로 Noti 소리나 진동이<br/>
   울리게 될 것이므로 불편이 전혀 없지는 않을 것입니다.<br/>
   그래서, 아래와 같은 방식으로 Noti를 처리하고자 했습니다.<br/>
   
   - 카카오톡은 알림을 끄면 소리와 진동만 끄고 알림표시는 여전히 하고 있는데<br/>
     여기서는 알림 표시도 하지 않기로 합니다. 또한, 기본적으로 한쪽에서 읽으면<br/>
     웹과 앱에 데이터를 보내 혹시 노티가 떠 있으면 닫히게 했습니다.<br/>
   ...
   - 앱에서는 1초 정도 늦게 해당 메시지가 읽혔는지 체크하고 나서 노티를 열지 판단합니다.<br/>
   - 그런데, 앱에서 챗방이 열려 있고 포커싱이 있는 상태라면 웹에서는 톡 올 때마다 노티가<br/>
     계속 표시되었다가 제거되는 것이 반복되는데 이것을 해결하기 위해 focusedRoomid가 빈칸이<br/>
     아니면 현재 앱에서 해당 roomid로 챗방이 맨위에 열려 있고 스크린도 On 상태라고 인지되므로,<br/>
     웹에서는 노티가 필요없게 했습니다.<br/>

   이 부분은 테스트해보기 -> 아래와 같이 안해도 해결되는거 같은데................. <br/>
