...
# 주요 특징 (메시징 관점에서)
   ...
   Notification(톡 도착 알림)과 관련하여 웹만 또는 앱만 사용시는 일반적인 사안이지만<br/>
   웹과 앱 동시에 사용하는 경우 Noti가 둘 다 동시에 오면 사용자가 불편하게 되므로<br/>
   아래의 로직으로 처리했습니다.<br/>
   ...
   - 그게 아니면, 해당 톡을 이미 읽었는지 (1초후) 체크해서 읽지 않았으면 알림을 표시할 요건이 됩니다.<br/>
     (카카오 노티는 알림을 끄면 소리와 진동만 끄고 알림표시는 여전히 하고 있는데 여기서는 알림표시도 하지 않기로 합니다)<br/>
   - 웹과 앱 모두 알림을 무조건 표시합니다.<br/>
     대신, 앱에서는 1초 늦게 해당 메시지아이디가 읽혔는지 체크하고 노티를 열지 판단합니다.<br/>
   - 그럴 경우, 그 반대로 앱에서 챗방이 열려 있고 포커싱이 있는 상태라면<br/>
     웹에서는 톡 올 때마다 노티가 계속 표시되었다가 제거되는 것이 반복되는데 이것을 해결하기 위해<br/>
     ocusedRoomid가 빈칸이 아니면 현재 앱에서 해당 roomid로 챗방이 맨위에 열려 있고 스크린도 On 상태이어서<br/>
     main_common.js의 focusedRoomid로 웹에서 앱으로 (단방향으로만) 요청/응답받는 소켓정보로 사용해<br/>
     focusedRoomid가 빈칸이 아니면 현재 앱에서 해당 roomid로 챗방이 맨위에 열려 있고 스크린도 On 상태이어서<br/>
     웹과 앱이 모두 접속된 상태에서 톡이 오면 앱에서는 계속 메시지가 읽음처리가 되므로<br/>
     웹에서는 노티가 필요없게 됩니다.<br/>
   - 기본적으로 한쪽에서 읽으면 웹과 앱에 데이터를 보내 혹시 노티가 열려 있으면 닫습니다.<br/>
   ...
   + 이 부분은 테스트해보기 -> 아래와 같이 안해도 해결되는거 같은데.................
     앱에서는 사용자에 의한 앱의 강제종료 및 사용자의 의도와 관련없이 네트워크 재연결이 자주 발생할 수도 있는데<br/>
     이 경우, 그 시간동안 새로운 톡 도착을 (안읽은 톡으로) 알려줘야 합니다.<br/>
     그런데, 웹과는 다르게 알림이 표시되고 나면 다음 네트워크 재연결이 되거나 강제 종료후 재구동시에는<br/>
     LASTCHKDT 필드값을 업데이트해서 그 이후에 온 안읽은 톡만 알림을 표시하는 것이 합리적일것으로 보임
...
POPUP_OFF CHAR(1) NOT NULL DEFAULT '' COMMENT 'POPUP숨기기(Y)',
