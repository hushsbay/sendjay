...
# 주요 특징 (프로그래밍 관점)
   ...
   * 모바일 네이티브앱 + 웹뷰의 경우
   ...
   sendjay는 유지보수 효율성을 목적으로 두고 모바일에서, 앱으로 반드시 구현해야 할<br/>
   소켓통신/알림/앱버전업데이트 등은 앱으로 개발하고 UI(화면) 관련은 대부분 웹뷰를 통해 기존에 개발된<br/>
   웹모듈을 재사용(공유)하고자 했습니다.<br/>
...
# 사내시스템내 sendjay 메시징 서버/웹/앱 구축하기
   ...
   * 아래는 구축 방식 비교표입니다.
   * sendjay 소스는 albahero.com:547에 그대로 적용되어 있는데 이는 테스트 사이트이므로<br/>
