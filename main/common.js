...
module.exports = (function() {
	...
	let com = {
		...
		makeToken : (userid, passkey) => {
			...
			const key = passkey + passkey + passkey + passkey + passkey + '!!' //32bytes (아래 verifyToken에서 동일하게 사용)
			const token = ws.token.makeJwt(obj, key)
			return token
		},

		verifyToken : async (userid, token) => {
			//Passkey는 1) 사용자등록시 저장되고 2) 웹메신저 설정 탭에서 Reset_Authentication(분실신고) 버튼 누르면 값이 변경되어 
			...
			try {
				...
				if (data.length == 0) throw new Error('인증실패입니다 : Userid(Cookie)' + ws.cons.MSG_NOT_EXIST)
				if (data[0].PASSKEY == '') throw new Error('Passkey가 DB에 저장되어 있지 않습니다.')
				if (conn) wsmysql.closeConn(conn)
				const key = data[0].PASSKEY + data[0].PASSKEY + data[0].PASSKEY + data[0].PASSKEY + data[0].PASSKEY + '!!' //32bytes (위 makeToken에서 동일하게 사용)
		...
		verifyWithRestUserId : async (req, res, idToCompare, _logTitle) => { //for rest only
			//대부분의 경우는 idToCompare와 userid쿠키는 일치해야 하는 경우가 많음 (특히, 특정 사용자아이디만 체크해야 하는데 쿠키랑 다르면 현재 사용자가 다른 userid로 뭔가 하려는 것이므로 막아야 할 대 사용). 
            //그게 아니라면, idToCompare를 null로 두면 (사용자쿠키가 누구이든 검증만 되면 비교대상없이) 통과될 것임 
			let result = true
			try {
				const rs = await com.verifyToken(req.cookies.userid, req.cookies.token)
				if (rs.code != ws.cons.RESULT_OK) {
					ws.http.resCodeMsg(res, rs.code, rs.msg, _logTitle)
					result = false
				} else {
					if (idToCompare && idToCompare != req.cookies.userid) {
						ws.http.resCodeMsg(res, ws.cons.WARN_USE_ONLY_OWN_USERID, 'mismatch between UserID(' + idToCompare + ') and CookieUserID(' + req.cookies.userid + ')', _logTitle)
						result = false
					} else {
						req.token = rs.token //req.token으로 받아서 비교 또는 처리하는데 사용하기 위한 목적임
					}
				}
			} catch (ex) {
				ws.log.ex(req, ex, _logTitle)
				ws.http.resCodeMsg(res, ws.cons.RESULT_ERR, ex.message, _logTitle)
				result = false
			} finally {
				return result //req.token을 요청한 곳으로 전달하기 위해 code/msg를 반환하지 않고 true/false로 반환하고 false일 경우는 아예 ws.http.resCodeMsg() = res.json()으로 처리해버림
			}
		},
		...
		chkAccessUserWithTarget : async (userid, uid, type, target) => { //for socket or rest
			...
			try { //console.log(userid, uid, type, target)
				...
				if (type == 'file' || type == '') { //'' means general type for msgid
					...
					if (data[0].CNT > 0) {
						if (type == 'file') {
							const dataM = await wsmysql.query(conn, "SELECT TYP TYPE, BODY FROM " + com.tbl.msgmst + " WHERE MSGID = ? ", [uid])
							if (dataM.length > 0) {
		...
		getRole : async (userid, conn) => {
			...
			try {
				if (userid) {
					const data = await wsmysql.query(conn, "SELECT USERROLE ROLE FROM " + com.tbl.user + " WHERE USER_ID = ? ", [userid])
		...
		sendToRoom : (socket, roomid, param) => {
			...
			socket.to(roomid).emit(com.cons.sock_ev_common, param) //본인 소켓 제외
			socket.emit(com.cons.sock_ev_common, param) //본인 소켓에게 보냄
		},
		...
		multiSetForUserkeySocket : async (socket) => {
			try {
				const usKey = com.cons.key_str_socket + socket.userkey + com.cons.easydeli + socket.id //예) $$S + W__userid + ; + XYZ~
				const uwKey = com.cons.key_str_winid + socket.userkey + com.cons.easydeli + socket.winid //예) $$W + W__userid + ; + 2023~
				...
				const arr = await global.store.multi().set(usKey, socket.socketid)
											  .set(uwKey, ws.util.getCurDateTimeStr(true)) //See chk_redis.js, too.
										   	  .sadd(com.cons.key_set_userkey_socket, usKey) //예) $$US에 $$S + W__userid + ; + XYZ~를 추가
										   	  .exec() //.sadd는 현재 미사용이나 향후를 위해 일단 그대로 둠 / .scard(com.cons.key_set_userkey_socket)
