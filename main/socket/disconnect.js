...
module.exports = async function(socket, reason) {	
	...
	try { //userkey made to null when prevsocket disconnect
		if (socket.userkey) { //이전 소켓을 끊으려고 하는 pmessage에서 socket.prev=true로 주기 : sock_ev_show_off가 sock_ev_show_on보다 늦게 처리되어 on/off가 off로 보이는 걸 막기 위함
			if (!socket.prev) ws.sock.broadcast(ws.cons.sock_ev_show_off, socket.userkey, 'all')
