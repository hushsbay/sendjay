		...
		if (_chan == 'disconnect_prev_sock') { //adapter.remoteDisconnect 사용하지 않음 : 아래 코딩처럼 처리할 내용이 있어서 그대로 사용하기로 함
			...
			if (prevSocket) { //Previous socket for current userkey exists in this server. 해당서버에 이전 소켓이 있으므로 연결 끊기.
				if (prevSocket.userkey.startsWith(com.cons.m_key)) { //Mobile App
					...
					prevSocket.disconnect() //redis 데이터 처리(com.multiDelForUserkeySocket())는 disconnect.js에서 담당
				...
				await com.multiDelGarbageForUserkeySocket(obj.prevkey, true) //소켓정보 없으므로 가비지로 처리 (every socket server)
			}
