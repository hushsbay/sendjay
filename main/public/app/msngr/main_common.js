...
const getMembers = async (type, keyword, tag) => { //group or search. (userids used in chat.html also)
    try {
        ...
        for (let i = 0; i < _len; i++) {
            ...
            _html += "          <div id=first_" + _userid + " style='flex-grow:1;display:flex;align-items:center;overflow:hidden;margin:3px 0px'>"
            ...
            _html += "              <span id=nm_" + _userid + " title='" + _nm + "' class='coDotDot coText' style='min-width:50px;max-width:120px;color:#005192;font-weight:bold;margin-left:5px'>" + _nm + "</span>"
...
const getOrgTree = async (obj) => { //예) const obj = { keyword : "", withMember : false, expand : 1 }
    ...
    try {
        ...
        for (let i = 0; i < _len; i++) {
            ...
            _html += "      <input type=checkbox id=orgsel_" + i + " class=orgsel z-index=1 style='" + chkDisp + "margin-left:0px' />"
...
const getPortalList = async (obj) => {
    try {  
        ...
        for (let i = 0; i < _len; i++) {
            ...
            _html += "              </div>"
            _html += "          </div>"
            _html += "          <div style='width:100%;height:50%;display:flex;align-items:flex-start;justify-content:center;margin:2px 0px'>"
            _html += "              <div id=body_" + _roomid + " class='coDotDot coText' style='flex:1;min-width:0;color:darkgray;vertical-align:middle'>" + _lastmsg + "</div>"
...
const getUnreadPerEachRoom = async (roomid, chkCloseNoti) => { //no unread display in case of invite/leave msg  
    ...
    const rs = await hush.http.ajax("/msngr/qry_unread", { roomid : roomid }, true)
