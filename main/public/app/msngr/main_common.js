...
const getMembers = async (type, keyword, tag) => { //group or search. (userids used in chat.html also)
    try {
        ...
        for (let i = 0; i < _len; i++) {
            ...
            _html += "              <span id=w_" + w_userkey + " class='state coStateOff coText'>W</span>"
            _html += "              <span id=m_" + m_userkey + " class='state " + state_mob + " coText' style='margin-left:5px'>M</span>"
...
const getPortalList = async (obj) => {
    try {  
        ...
        for (let i = 0; i < _len; i++) {
            ...
            _html += "              <div class=coDotDot style='flex:1;min-width:0;color:#005192;font-weight:bold'>" + _roomnm + "</div>" //flex:1;min-width:0 used instead of width:calc
            ...
            _html += "                  <span id=unread_" + _roomid + " style='display:none;border-radius:8px;background-color:orange;color:white;padding:2px 6px;margin-left:10px'></span>"
