(function($) {
    ...
    window.hush = {
        ...
        blob : { //브라우저에서 이미지 파일 선택후 노드서버에 올리고 다시 내려 표시하는 등 처리는 2가지 방법이 있음 => 1) blob 2) base64인코딩스트링
            ...
            get : async (url) => { //get(getPromise)도 <img>의 src url이 blob이든 base64든 관계없이 blob 데이터 추출해서 서버로 보낼 준비를 함
                try {
        ...
        msg : { //1. msg(비동기콜백) 2. alert(=window.alert) 3. confirm(=window.confirm) 4. toast(복수메시지 순서대로 표시 지원)
            ...
            addHtml : (_type, _text, _callbackOk, _callbackCancel, _obj) => { //_obj (toast에는 미사용) : width, height, backColor, color
                ...
                _html += "		    <div style='width:100%;height:calc(100% - 45px);overflow-wrap:break-word;overflow:auto'>" + _text.replaceAll("\n", "<br>") + "</div>"
            ...
            toast : (_text, _sec) => {
                hush.msg.toastTextArr.push(_text.replaceAll("\n", "<br>"))
